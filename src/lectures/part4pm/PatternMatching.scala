...
object PatternMatching extends App {
   ...
   */
  trait Expr //some math expresion
  case class Number(n: Int) extends Expr
  case class Sum(e1: Expr, e2: Expr) extends Expr
  case class Prod(e1: Expr, e2: Expr) extends Expr

  val myExpr = Prod(Sum(Number(3), Number(4)), Number(2))

  def printItOut(expr: Expr): String = {
    expr match {
      case Number(n) => s"$n"
      case Sum(e1, e2) => "(" + printItOut(e1) + " + " + printItOut(e2) + ")"
      case Prod(e1, e2) => "(" + printItOut(e1) + " * " + printItOut(e2) + ")"
    }
  }
